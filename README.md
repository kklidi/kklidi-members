...
**현재 개발 후보: 0.7.13.** 0.7.13은 비밀번호 재설정 화면에 이메일·사용자명 입력과 WordPress Core의 generic reset-link 안내를 명확히 표시한다. 0.7.12의 가입 후 수동 로그인 정책, 0.7.11의 탈퇴 검토·처리 UX, `AUTH-NOTIFY-002`의 plain-text 제목·본문 설정, WordPress Core 인증과 사용자 ID 경계는 그대로 유지한다. 실제 운영 인증서·다중 서버·운영 migration 승인은 [운영 계약](docs/OPERATIONS.md)의 환경 gate를 따른다.
