...
**현재 개발 후보: 0.7.11.** 0.7.11은 탈퇴의 즉시 로그인 차단·검토·처리 단계를 화면에 명확히 표시하고, 계정 알림 설정의 공란이 수신자 언어별 기본 문구를 사용한다는 점을 안내한다. `AUTH-NOTIFY-002`의 plain-text 제목·본문 설정, WordPress Core 인증과 사용자 ID 경계, 사이트 메일 전송 계층 소유권은 그대로 유지한다. 실제 운영 인증서·다중 서버·운영 migration 승인은 [운영 계약](docs/OPERATIONS.md)의 환경 gate를 따른다.
