...
**현재 개발 후보: 0.7.12.** 0.7.12는 가입 완료 후 자동 로그인 없이 이메일 또는 사용자명으로 직접 로그인하는 정책을 가입·로그인 화면에 명확히 표시한다. 0.7.11의 탈퇴 검토·처리 UX와 `AUTH-NOTIFY-002`의 plain-text 제목·본문 설정, WordPress Core 인증과 사용자 ID 경계, 사이트 메일 전송 계층 소유권은 그대로 유지한다. 실제 운영 인증서·다중 서버·운영 migration 승인은 [운영 계약](docs/OPERATIONS.md)의 환경 gate를 따른다.
