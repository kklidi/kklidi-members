...
**다음 0.7.3 개발 상태:** `AUTH-NOTIFY-001` P0-4까지 완료했다. 네 계정 사건의 `wp_mail()` 실패에서도 확정 상태·세션 철회·성공 응답이 유지되고 실패 감사만 남으며 전달 기록·자동 재시도는 0건임을 두 prefix의 합성 WordPress에서 검증했다. 0.7.3 배포 ZIP과 버전 표시는 아직 만들지 않는다.
