...
**현재 개발 후보: 0.7.23.** 0.7.23은 로그인·비밀번호 찾기 화면을 중앙 카드로 정리하고 불필요한 사이트명 eyebrow를 제거한다. 로그인 하단 링크는 회원가입·비밀번호 찾기·홈의 단일 행으로 통일한다. 0.7.22의 입력 필드·필수 표시·간결한 문구 계약과 0.7.21의 고정 `/members/` route, WordPress Core 인증·사용자 ID·가입 공개 정책·동의·메일 소유권 경계는 그대로 유지한다. 실제 운영 인증서·다중 서버·운영 migration 승인은 [운영 계약](docs/OPERATIONS.md)의 환경 gate를 따른다.
...
| [MESSAGES](docs/MESSAGES.md) | `AUTH-MESSAGE-UX-001`의 읽기 전용 기본 문구 catalog와 이메일 기본값 표시 경계 |
| [ADMIN_NOTIFICATIONS](docs/ADMIN_NOTIFICATIONS.md) | `AUTH-ADMIN-NOTIFY-001`의 선택적 신규 가입 관리자 알림 경계 |
| [ROUTE_MANAGEMENT](docs/ROUTE_MANAGEMENT.md) | `AUTH-ROUTE-MAP-001`의 shortcode 없는 clean route 활성화·충돌·fallback 경계 |
...
`AUTH-UI-002`와 후속 `AUTH-UX-003` 구현은 9개 frontend Members route와 Members 관리자 화면에 공통 page shell 및 stylesheet를 적용한다. frontend stylesheet는 Members query route에만, admin stylesheet는 `Tools → KKLIDI Members` 화면에만 로드된다. password 표시 JavaScript도 로그인·가입·변경·재설정 route에만 로드되며 외부 font/CDN 요청은 없다.\n
