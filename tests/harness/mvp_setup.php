...
if ($mode === 'core-on-no-documents') {
    ...
    echo json_encode(array('required_ready' => Documents::required_ready(),
        'legacy_registration_option' => get_option('kklidi_members_registration_enabled'),
...
if ($mode === 'documents-ready-core-off') {
    update_option('users_can_register', 0);
    update_option('kklidi_members_registration_enabled', '1', false);
} elseif ($mode === 'core-on') {
    update_option('users_can_register', 1);
    update_option('kklidi_members_registration_enabled', '0', false);
} else {
    exit('Invalid setup mode.');
}
