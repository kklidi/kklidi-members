...
`mamp_race_run.py` exercises eight simultaneous registration submissions and eight pre-opened Core reset forms against Apache workers. `mamp_timing_run.py` records bounded existing/missing-identifier timing distributions without retaining identifiers. `reference_d06_audit.php` reads only structural metadata inside a rolled-back read-only transaction. `deployment_preflight.py` refuses HTTP and credentials, then checks trusted HTTPS, HSTS, private/no-store headers, and the guest-cookie flags.

**Remaining environment gates:** production TLS/Core Secure auth-cookie behavior and a persistent-cache/proxy outage matrix require the actual deployment stack. Distributed multi-node abuse and production migration also require an explicitly identified staging/production environment and owner handoff. KBoard permission-engine parity and content migration remain out of scope because KBoard is scheduled for removal.
