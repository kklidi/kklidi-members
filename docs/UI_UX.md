...
## 18. AUTH-REGISTER-UX-007 · 0.7.24 필수 입력 표시 계약

회원가입 필수 정책은 `RegistrationFields`의 고정 필수 필드와 이름·성·전화의 설정 상태를 단일 서버 기준으로 사용한다. 이메일·비밀번호·비밀번호 확인·표시명·필수 동의는 관리자에서 숨길 수 없는 `필수 · 고정` 필드로 표시한다. 라벨의 별표는 CSS 구조 추론이 아니라 서버 렌더링 `<span>`으로 출력하고 `aria-hidden`을 사용한다. 폼 상단에는 `* 필수 입력` 범례를 제공하며 선택 상태 필드의 `선택` 안내는 유지한다.

## 19. AUTH-WITHDRAW-UX-008 · 0.7.25 탈퇴 재인증과 보존 안내 계약

탈퇴 요청은 WordPress Core 해시를 `wp_check_password`로 재확인하며 실패 시 비밀번호 필드에 오류와 `aria-invalid`를 함께 표시한다. 비밀번호 입력에는 기존 route 전용 눈 아이콘을 사용하고 실패 후 비밀번호 값을 복원하지 않는다. 제출 직후 로그인은 차단되며, 화면 문구는 삭제 대상 데이터가 해당 보존 기간에 따라 처리된다는 점과 법률·서비스 정책상 보존될 수 있는 기록을 함께 안내한다. Members가 모든 도메인의 전체 데이터 삭제를 보장하는 문구는 사용하지 않는다.
