...
| D11 | **IMPLEMENTED FOR 0.7.18 2026-09-10** | `AUTH-REGISTER-FIELDS-001`: 이메일·비밀번호·표시명·필수 동의는 잠그고 이름·성·전화만 필수/선택/숨김으로 설정. 현재 동작을 기본값으로 보존하며 임의 custom field·meta key·순서·label 편집과 profile/Woo/LMS 변경은 포함하지 않음 |
| D12 | **IMPLEMENTED FOR 0.7.19 2026-09-10** | `AUTH-MESSAGE-UX-001`: 보안·상태 문구는 gettext 기반 읽기 전용 catalog로 보여주고, 네 사용자 이메일은 Notifications에서 번역 기본 제목·본문을 명시적으로 표시. 전체 문구 editor, 새 message option, HTML과 관리자 가입 알림은 포함하지 않음 |
| D13 | **IMPLEMENTED FOR 0.7.20 2026-09-10** | `AUTH-ADMIN-NOTIFY-001`: 신규 가입 관리자 알림은 기본 꺼짐 opt-in. 활성 가입 완료 뒤 Core `admin_email`에 plain text로 1회 요청하고 실패는 가입을 rollback하지 않음. 승인제·복수/사용자 정의 수신자·HTML·SMTP·retry는 포함하지 않음 |
| D14 | **IMPLEMENTED FOR 0.7.21 2026-09-10** | `AUTH-ROUTE-MAP-001`: WordPress 페이지·shortcode를 만들지 않고 아홉 고정 `/members/` clean route를 선택적으로 활성화. 기본은 꺼짐이고 pretty permalink·namespace/page/rewrite 충돌을 무변경 거부하며 query route와 Core `force_reauth` fallback을 보존. custom slug·메뉴 자동 변경·범용 접근 제한은 포함하지 않음 |
| D15 | **IMPLEMENTED FOR 0.7.28 2026-09-10** | `AUTH-IDENTITY-002`: 신규 가입은 별도 아이디 없이 이메일 중심으로 유지하고 비공개 Core `user_login`과 독립된 공개 `user_nicename`을 자동 생성. 로그인·reset UI는 이메일을 주 label로 안내하되 기존 WordPress 아이디와 이메일 인증을 모두 유지하며 기존 user ID/login을 변경·병합하지 않음 |
| D16 | **IMPLEMENTED FOR 0.7.29 2026-09-10** | `AUTH-MAIL-SENDER-001`: 기본 꺼짐의 Members 전용 발신 이름·주소와 plain-text footer, 현재 관리자 이메일로 제한한 시험 발송. Members `wp_mail()` header에만 적용하고 전역 sender filter, SMTP/provider credential, HTML, Core reset·Woo/LMS/KBoard 메일 변경은 금지 |

D03~D05는 보수적 기본값으로 확정했다. 마케팅 수집이나 강제 재동의를 실제로 시작하려면 새로운 목적·보존·철회 behavior contract가 필요하다. FUTURE/OPTIONAL 기능은 별도 승인 없이 구현하지 않는다. D09의 상세 계약과 단계별 구현 상태는 `UI_UX.md`와 `tests/harness/ux_strategy_contract.json`이 소유한다. D10의 Core 우선 관리자 설정 경계는 `NOTIFICATIONS.md`와 `tests/harness/notification_settings_contract.json`이 소유한다. D11의 제한된 가입 필드 설정은 `REGISTRATION_FIELDS.md`와 `tests/harness/registration_fields_contract.json`이 소유한다. D12의 메시지 표시 경계는 `MESSAGES.md`와 `tests/harness/message_ux_contract.json`, D13의 관리자 가입 알림은 `ADMIN_NOTIFICATIONS.md`와 `tests/harness/admin_notification_contract.json`, D14의 page 비의존 route 연결은 `ROUTE_MANAGEMENT.md`와 `tests/harness/route_management_contract.json`이 소유한다. D15는 `IDENTITY_POLICY.md`와 `tests/harness/identity_policy_contract.json`, D16은 `MAIL_SENDER_POLICY.md`와 `tests/harness/mail_sender_contract.json`이 소유한다. D15와 D16은 각 runtime 합격 기준과 외부 배포 gate를 함께 따른다.
