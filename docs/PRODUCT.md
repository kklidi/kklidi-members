...
| D14 | **SPECIFIED FOR 0.7.21 2026-09-10** | `AUTH-ROUTE-MAP-001`: WordPress 페이지·shortcode를 만들지 않고 아홉 고정 `/members/` clean route를 선택적으로 활성화. 기본은 꺼짐이고 namespace/page/rewrite 충돌을 무변경 거부하며 query route와 Core `force_reauth` fallback을 보존. custom slug·메뉴 자동 변경·범용 접근 제한은 포함하지 않음 |
