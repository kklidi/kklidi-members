...
0.7.2 합성 rehearsal은 dry-run→import→동일 batch 재실행→`legacy_wpmembers` import row만 제한적으로 제거→재실행 가능 상태를 검사한다. Core ID와 source meta는 전 과정에서 보존한다. 실제 전환은 [OPERATIONS](OPERATIONS.md)의 credential-free deployment manifest에 담당 역할, backup hash, 별도 restore 시험, route owner, rollback 방법과 최소 관찰 조건을 기록한 뒤 실행한다.
