...
| AUTH-ADMIN-NOTIFY-001 / 0.7.20 extension (SPECIFIED) | default-off Settings API, Core `admin_email`, post-active plain-text 발송, 중복·실패·권한·금지정보 | 꺼짐 무발송; 켜짐 가입 요청당 1회; invalid recipient/wp_mail failure가 가입을 rollback하지 않음; 승인제·custom recipient/template·HTML/SMTP 없음 |
