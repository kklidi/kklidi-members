...
| AUTH-MESSAGE-UX-001 / 0.7.19 extension (SPECIFIED) | 관리자 기본 message catalog, 네 이메일 번역 기본값 표시, 일반 회원 접근, 비밀번호 변경 완료 redirect | 보안 문구 읽기 전용·새 option 없음; email override/fallback schema 불변; 비밀 없는 상태값과 재로그인; frontend 일반 요청 load 불변 |
