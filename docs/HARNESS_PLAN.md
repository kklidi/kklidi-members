...
| AUTH-ROUTE-MAP-001 / 0.7.21 extension (SPECIFIED) | default-off clean route, namespace/page/rewrite 충돌, 관리자 권한·nonce, query fallback, Core URL ownership, Members-off | 설계 가드만 존재. shortcode/page/menu 자동 생성 없이 아홉 고정 route를 같은 controller에 연결하고, 충돌 시 무변경 거부·일반 요청 flush/자산 0을 runtime에서 검증해야 함 |
