...
| AUTH-REGISTER-FIELDS-001 / 0.7.18 extension (IMPLEMENTED) | 기본 설정, 이름·성·전화의 필수/선택/숨김 조합, 비관리자 설정 시도, hidden field·role·ID·meta 주입, invalid schema/state | `wp_`/임의 prefix 합성 PASS: 0.7.17 기본 동작 보존; 내장 세 필드만 변경; 숨김 입력 무시; invalid update 전체 거부; Core ID/auth·동의·profile·Woo/LMS 데이터 불변 |
| AUTH-MESSAGE-UX-001 / 0.7.19 extension (IMPLEMENTED) | 관리자 기본 message catalog, 네 이메일 번역 기본값 표시, 일반 회원 접근, 비밀번호 변경 완료 redirect | `76b5fbbeec1940548eea447f3af980dd` 기본/임의 prefix 합성 PASS: 16개 key 읽기 전용·새 option 없음; email override/fallback schema 불변; 비밀 없는 상태값과 재로그인; frontend 일반 요청 load 불변 |
| AUTH-ADMIN-NOTIFY-001 / 0.7.20 extension (IMPLEMENTED) | default-off Settings API, Core `admin_email`, post-active plain-text 발송, 중복·실패·권한·금지정보 | `b83fd4753406440a874f0e50e55d0860` 기본/임의 prefix·성능 합성 PASS: 꺼짐 무발송; 켜짐 가입 요청당 1회; invalid recipient/wp_mail failure가 가입을 rollback하지 않음; 승인제·custom recipient/template·HTML/SMTP 없음 |
| AUTH-IDENTITY-002 / 0.7.28 extension (IMPLEMENTED) | 신규 가입 username 미수집, 비공개 Core login과 독립 공개 nicename 생성, 이메일 주 UI, 기존 username/email 로그인·reset 호환 | 신규 이메일 로그인과 별도 username 기존 계정 양쪽 성공; 표시명 인증 거부; generic 오류·limiter 유지; user ID/login 불변; Members-off Core login no-fatal |
| AUTH-MAIL-SENDER-001 / 0.7.29 extension (IMPLEMENTED) | default-off sender/footer, Members별 From header, 관리자 본인 제한 시험 발송, invalid/CRLF/권한·nonce·rate limit | 실행 `2fb34a1a46b14894989c36e755b2dbd1`에서 기본/임의 prefix 경계를, 격리 MAMP 실행 `mamp-mail-sender-56dbd5af1df2`에서 실제 WordPress `wp_mail()` 수락·plain-text header/footer·감사·정리를 확인. 실제 mailbox 도착·SPF/DKIM/DMARC 정렬은 운영 gate |
| AUTH-ROUTE-MAP-001 / 0.7.21 extension (IMPLEMENTED) | default-off clean route, pretty permalink·namespace/page/rewrite 충돌, 관리자 권한·nonce, query fallback, Core URL ownership, Members-off | `8c6ea0415d3d4e208ed79dbc7727886b` 기본/임의 prefix·성능 합성 PASS. 고정 MAMP 후보 runner도 실제 Apache rewrite의 아홉 경로, query fallback, 일반 요청 무자산과 option/permalink/rewrite/`.htaccess`/page/menu/audit/플러그인 tree 원복을 PASS해야 릴리스 evidence에 포함 |
...
2026-09-10 0.7.18 `AUTH-REGISTER-FIELDS-001`: Settings API의 non-autoload option으로 이름·성·전화만 필수/선택/숨김 처리한다. `10c0655fedec48139a72e44264503bc5`에서 기본/임의 prefix, 관리자·subscriber 권한, invalid state 원자 거부, server required 검증, hidden POST 무시, fixed subscriber와 합성 user cleanup을 PASS했다.
2026-09-10 0.7.19 `AUTH-MESSAGE-UX-001`: `76b5fbbeec1940548eea447f3af980dd`에서 관리자 전용 읽기 catalog 16개, 보안 문구 저장 경로 부재, 네 plain-text 이메일의 번역 기본값, 비밀번호 변경 뒤 비밀 없는 로그인 안내와 수동 로그인을 기본/임의 prefix에서 PASS했다.
2026-09-10 0.7.20 `AUTH-ADMIN-NOTIFY-001`: `b83fd4753406440a874f0e50e55d0860`에서 opt-in Settings API, Core `admin_email`, 활성 가입 뒤 plain-text 관리자 메일, 사용자 메일과의 분리, 논리 중복 방지, invalid recipient, 실패 시 가입 보존, 일반 요청 성능 budget을 기본/임의 prefix에서 PASS했다.

2026-09-10 0.7.21 `AUTH-ROUTE-MAP-001`: `8c6ea0415d3d4e208ed79dbc7727886b`에서 default-off와 pretty permalink 전제, page/rewrite 충돌 무변경 거부, 관리자 권한·nonce, non-autoload strict schema, 아홉 clean route와 query fallback, Core URL ownership·force-reauth, rollback·손상 설정 fallback을 기본/임의 prefix에서 PASS했다. paired 일반 페이지 측정은 중앙값 2.922ms, p95 1.369ms 증가로 budget 안이었고 메모리·전역 자산은 불변, query 중앙값은 route option 확인으로 4→5였다.

0.7.21 고정 MAMP 후보 runner는 릴리스 ZIP을 임시 장착한 동안 route, TLS/Core cookie, 실제 LMS, Woo checkout/cart/order/device-limit, KBoard on/off, 가입·Core reset 경쟁과 login timing runner를 직렬 실행한다. 각 runner가 PASS하고 기존 Members tree와 임시 디렉터리가 복원된 경우에만 후보 전체를 PASS한다. 운영 인증서/CDN/proxy와 실제 mailbox 전달은 이 로컬 판정에 포함하지 않는다.

2026-09-10 0.7.30 실제 Chrome 회귀 검사: `mamp-browser-0.7.30-20260910`에서 데스크톱 로그인 중앙 정렬과 인라인 눈 아이콘, 360px 가입 필수 표시·가로 overflow 부재, 로그인 회원의 계정·프로필·비밀번호·동의·탈퇴·로그아웃 화면, 관리자 알림 기본값과 Members 발신자/footer 설정을 확인했다. 이 과정에서 탈퇴 route의 password enhancement script 누락을 발견해 route 목록을 보정했고, 수정 후 버튼 표시·SVG·`password`↔`text` 전환·ARIA 상태를 재검증했다. 합성 사용자, 임시 공개가입 설정과 약관 snapshot은 모두 제거하고 원래 닫힌 상태로 복원했다. 증거는 `.harness/reports/mamp-browser-0.7.30.json`이다.
