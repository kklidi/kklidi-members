...
6. 알림은 최종 보안 상태가 저장된 뒤 발생한다. `wp_mail()` 실패로 계정 상태, 세션 철회, 비밀번호 변경을 rollback하지 않는다. 실패를 전달 완료로 표시하지 않으며 최소 감사 사건만 남긴다. 실패 주입, 사용자 응답, 재시도 여부의 세부 계약은 P0-4에서 확정한다.
...
- **P0-2:** 네 사건만 실제 성공 경로에서 발송하고, 잘못된 상태·실패·중복 요청에서는 논리 알림이 생기지 않음을 합성 WordPress에서 검증한다. 수신자, 금지 정보, WordPress ID 보존, Members 비활성 fallback도 검사한다.
