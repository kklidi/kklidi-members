...
운영 메일의 SPF·DKIM·DMARC와 실제 받은편지함 전달은 `docs/deployment/MAIL_AUTH_CHECKLIST.md`로 확인한다. Members는 공개 DNS나 SMTP/provider 설정을 자동 변경하지 않으며 실제 메일 원문을 저장소에 보관하지 않는다.
